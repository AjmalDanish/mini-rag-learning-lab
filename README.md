...
That answer was generated by a real LLM (glm-5 via OpenCode Go), grounded in context retrieved from the vector DB — not from the model's memory.
...
- **LLM:** glm-5 via OpenCode Go (any OpenAI-compatible provider works — OpenAI, Gemini, Groq, or a custom base URL; the notebook lists the models your key can access)
